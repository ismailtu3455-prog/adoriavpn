...
async def deliver_vpn(bot: Bot, user_id: int, days: int, is_purchase: bool = False, gateway: str = "Неизвестно") -> bool:
    ...
    try:
        ...
        if is_purchase:
            from ..config import db_settings
            pch = db_settings.get("purchase_channel_id")
            if pch:
                import datetime
                ...
                msg_pch = (
                    ...
                    f"💳 Способ: <b>{gateway}</b>\n"
                    f"📅 Дата: {dt}"
                )
                try:
                    await bot.send_message(pch, msg_pch)
                except Exception as e:
                    log.error(f"Failed to send to purchase channel: {e}")

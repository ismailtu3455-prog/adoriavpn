...
@router.callback_query(F.data == "adm:set_purchase_channel")
async def cb_set_purchase_channel(c: CallbackQuery, state: FSMContext):
    bot_me = await c.bot.me()
    await c.message.edit_text(
        "🛒 <b>Настройка канала успешных покупок</b>\n\n"
        "Отправьте пересланное сообщение из вашего канала.\n"
        "⚠️ <b>Обязательно:</b> Бот должен быть администратором в этом канале с правом публикации сообщений.\n"
        "Для удобства можете нажать кнопку ниже, чтобы добавить бота в свой канал.\n\n"
        "Отправьте 0, чтобы отключить этот канал.",
        reply_markup=inline.admin_channel_setup_kb(bot_me.username)
    )
    await state.set_state(AdminSettingsState.wait_for_purchase_channel)

@router.message(AdminSettingsState.wait_for_purchase_channel)
async def process_purchase_channel(m: Message, state: FSMContext):
    if m.text and m.text.strip() == "0":
        await crud.set_setting("purchase_channel_id", None)
        db_settings["purchase_channel_id"] = None
        await m.answer("✅ Канал успешных покупок отключен!", reply_markup=inline.admin_menu_kb())
        await state.clear()
        return
    ...
    await crud.set_setting("purchase_channel_id", val)
    db_settings["purchase_channel_id"] = val
    await m.answer("✅ Канал успешных покупок обновлен!", reply_markup=inline.admin_menu_kb())
    await state.clear()
...
@router.callback_query(F.data.startswith("adm:manual:approve:"))
async def cb_manual_approve(c: CallbackQuery):
    invoice_id = c.data.split(":")[3]
    inv = await crud.get_invoice(invoice_id)
    if not inv or inv.status != "pending_manual":
        return await c.answer("Счет уже обработан или не найден", show_alert=True)
    ...
    if inv.is_gift:
        ...
        try:
            await c.bot.send_message(
                inv.user_id,
                f"✅ Оплата подтверждена!\nВаш подарочный ключ готов.",
                reply_markup=inline.share_gift_kb(code, inv.days, bot_info.username, "Пользователь")
            )
        except Exception:
            pass
    else:
        from ..services.delivery import deliver_vpn
        await deliver_vpn(c.bot, inv.user_id, inv.days, is_purchase=True, gateway="Перевод по реквизитам")
        await crud.process_referral_bonus(c.bot, inv.user_id, inv.amount)
        try:
            await c.bot.send_message(inv.user_id, f"✅ Ваш платеж на {inv.amount} ₽ подтвержден!\nВам выдано {inv.days} дней VPN.", reply_markup=inline.main_menu())
        except Exception:
            pass
            
    await c.message.edit_reply_markup(reply_markup=None)
    await c.message.reply(f"✅ Выдано юзеру {inv.user_id}")
    await c.answer()

@router.callback_query(F.data.startswith("adm:manual:reject:"))
async def cb_manual_reject(c: CallbackQuery):
    invoice_id = c.data.split(":")[3]
    inv = await crud.get_invoice(invoice_id)
    if not inv or inv.status != "pending_manual":
        return await c.answer("Счет уже обработан или не найден", show_alert=True)
    ...
    try:
        await c.bot.send_message(inv.user_id, f"❌ Ваш платеж на {inv.amount} ₽ по тарифу {inv.plan} ({inv.days} дней) был отклонен администратором. Если произошла ошибка, обратитесь в поддержку.")
    except Exception:
        pass
        
    await c.message.edit_reply_markup(reply_markup=None)
    await c.message.reply(f"❌ Платеж отклонен для юзера {inv.user_id}")
    await c.answer()

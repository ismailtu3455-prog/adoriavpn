...
async def cmd_start(m: Message, command: CommandObject):
    ...
    if main_channel_id:
        ...
        try:
            ...
            pass # Если бот не админ, пропускаем
            
        if not is_subbed or not user or not user.tos_accepted:
...
async def _apply_promo(m: Message, code: str, state: FSMContext):
    ...
    has_used = await crud.has_user_used_promo(m.from_user.id, code)
    if has_used:
    ...
    if promo.promo_type == "days":
        ...
        await crud.record_promo_usage(m.from_user.id, code)
        is_adm = await is_admin(m.from_user.id)
        await m.answer(f"✅ Промокод успешно активирован! Вам добавлено {int(promo.value)} дней.", reply_markup=inline.main_menu(is_adm))
    elif promo.promo_type == "balance":
        await crud.add_user_balance(m.from_user.id, promo.value)
        await crud.increment_promo_uses(code)
        await crud.record_promo_usage(m.from_user.id, code)
        is_adm = await is_admin(m.from_user.id)
        await m.answer(f"✅ Промокод успешно активирован! Ваш баланс пополнен на {promo.value} ₽.", reply_markup=inline.main_menu(is_adm))
    elif promo.promo_type == "discount":
        await crud.update_user_promo(m.from_user.id, code)
        await crud.increment_promo_uses(code)
        await crud.record_promo_usage(m.from_user.id, code)
        is_adm = await is_admin(m.from_user.id)
        await m.answer(f"✅ Промокод на скидку {int(promo.value)}% активирован! Скидка применится при следующей покупке.", reply_markup=inline.main_menu(is_adm))

...
def payment_methods_kb(plan: str, balance: float = 0, price: float = 0, is_gift: bool = False) -> InlineKeyboardMarkup:
    ...
    builder.row(InlineKeyboardButton(text="💳 Перевод по реквизитам", callback_data=f"{prefix}:{plan}:manual"))
...
def admin_channels_setup_kb() -> InlineKeyboardMarkup:
    ...
    builder.row(InlineKeyboardButton(text="🛒 Канал покупок", callback_data="adm:set_purchase_channel"))
...
def manual_pay_decision_kb(invoice_id: str) -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    builder.row(
        InlineKeyboardButton(text="✅ Принять и выдать", callback_data=f"adm:manual:approve:{invoice_id}"),
        InlineKeyboardButton(text="❌ Отклонить", callback_data=f"adm:manual:reject:{invoice_id}")
    )
    return builder.as_markup()

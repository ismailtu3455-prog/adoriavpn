...
async def init_db():
    async with engine.begin() as conn:
        ...
        try:
            ...
            pass
        try:
            await conn.execute(text("ALTER TABLE invoices ADD COLUMN gateway_amount FLOAT"))
        except Exception:
            pass
        try:
            await conn.execute(text("ALTER TABLE invoices ADD COLUMN gateway_currency VARCHAR"))
        except Exception:
            pass
...
async def has_user_used_promo(user_id: int, code: str) -> bool:
    ...
    async with AsyncSessionLocal() as session:
        usage = await session.scalar(select(PromoUsage).where(PromoUsage.user_id == user_id, PromoUsage.code == code))
        return usage is not None

async def record_promo_usage(user_id: int, code: str):
    from .models import PromoUsage
    async with AsyncSessionLocal() as session:
        session.add(PromoUsage(user_id=user_id, code=code))
        await session.commit()

async def delete_promocode(code: str):
    async with AsyncSessionLocal() as session:
        from .models import PromoUsage
        await session.execute(delete(PromoUsage).where(PromoUsage.code == code))

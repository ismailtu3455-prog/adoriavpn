...
class Invoice(Base):
    ...
    gateway_amount: Mapped[float | None] = mapped_column(Float, nullable=True) # How much user paid in gateway currency
    gateway_currency: Mapped[str | None] = mapped_column(String, nullable=True) # "XTR", "RUB", etc
...
class PromoUsage(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    user_id: Mapped[int] = mapped_column(ForeignKey("users.user_id"), nullable=False)
    code: Mapped[str] = mapped_column(ForeignKey("promocodes.code"), nullable=False)
    created_at: Mapped[datetime.datetime] = mapped_column(DateTime, server_default=func.now())
